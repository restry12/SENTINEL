...
export interface SentinelUpdate {
  timestamp: string
  fires: Array<{ lat: number; lon: number; frp: number; brightness: number; timestamp: string }>
  polygon: object
  weather: { speed: number; deg: number; humidity: number }
...
export function useSocket() {
  ...
  useEffect(() => {
    ...
    // New backend emits "update" with full SentinelUpdate
    socket.on("update", (data: SentinelUpdate) => {
      setSentinelUpdate(data)
      // Map fires to Hotspot format for map compatibility
      setHotspots(data.fires.map((f, i) => ({
        id: `FIRE-${String(i + 1).padStart(3, "0")}`,
        lat: f.lat,
        lng: f.lon,
        intensity: data.riskLevel,
        frp: f.frp,
        label: `FIRE-${String(i + 1).padStart(3, "0")}`,
      })))
    })

    // Legacy event — old backend/server.js compatibility
  ...
  return { hotspots, sentinelUpdate, connected }
}
