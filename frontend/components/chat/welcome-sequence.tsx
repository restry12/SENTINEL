...
const WELCOME_TEXT =
  "Hola, soy Cóndi. Estoy monitoreando incendios, calidad del aire y viento extremo en tiempo real. Cuéntame qué está pasando cerca de ti y te ayudaré paso a paso."
...
export function WelcomeSequence({ onSuggestion, mode }: Props) {
  ...
  // Phase machine: entry → landing → speaking → idle
  useEffect(() => {
  ...
  // Condor wrapper: transitions position + size center → top-left corner
  ...
  return (
    <div className="relative w-full" style={{ minHeight: '480px' }}>
      ...
      {/* ── Cóndi avatar — absolutely positioned, animates center → side ── */}
      <div
      ...
      {/* ── Speaking phase: typewriter welcome text ── */}
      <div className={cn(
        "absolute left-4 right-4 flex flex-col items-center transition-all duration-500 ease-out",
        "top-[190px]",
        phase === 'speaking'
        ...
        <div className="max-w-[500px] text-center">
          <div className="flex items-center justify-center gap-2.5 mb-3">
            ...
            <span className="text-[9px] font-mono font-bold uppercase tracking-[0.28em] text-cyan-400/45">
              Cóndi dice
            </span>
      ...
      {/* ── Idle phase: title row + cards + chips ── */}
      <div className={cn(
        "absolute left-0 right-0 transition-all duration-700 ease-out",
        "top-[56px]",
        phase === 'idle'
          ? 'opacity-100 translate-y-0 pointer-events-auto'
          : 'opacity-0 translate-y-3 pointer-events-none'
      )}>
        {/* Title — offset right of the small condor */}
        <div className="ml-[62px] mb-5">
          <h2 className="text-[17px] font-black text-white tracking-tight leading-tight">
            ¿Qué está ocurriendo cerca de ti?
          </h2>
