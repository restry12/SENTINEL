...
export function TornadoLeftPanel({ gridData, selectedPoint, detail, detailLoading, onPointSelect, onBack }: Props) {
  return (
    <div className="absolute top-6 left-6 z-40 w-72 pointer-events-none h-[calc(100vh-160px)]">
      <div className="h-full overflow-y-auto pr-1 scrollbar-none pointer-events-auto flex flex-col gap-3 pb-12">
        {selectedPoint ? (
          ...
          <GlobalPanel gridData={gridData} onPointSelect={onPointSelect} />
        )}
      </div>
    </div>
  )
...
function GlobalPanel({ gridData, onPointSelect }: { gridData: GridScanResult | null; onPointSelect: (p: GridPoint) => void }) {
      ...
      <div className="bg-[#0a0d14]/90 backdrop-blur-xl border border-white/10 rounded-lg p-4 shadow-2xl">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/50">
            {highRisk.length > 0 ? "Severe Weather Detected" : "Global Monitor Active"}
          </span>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-black/30 rounded border border-white/5 p-2.5">
            ...
            <p className="text-xl font-black text-white">{gridData?.metadata.total_points ?? 0}</p>
          </div>
      ...
      {highRisk.length > 0 && (
        <div className="bg-[#0a0d14]/90 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-white/[0.06] flex items-center gap-2">
            ...
            <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/50">High Risk Zones</span>
          </div>
          <div className="p-3 flex flex-col gap-2 max-h-64 overflow-y-auto scrollbar-none">
            {highRisk.sort((a, b) => b.score - a.score).slice(0, 10).map((point, i) => {
...
function PointDetailPanel({ point, detail, loading, onBack }: { point: GridPoint; detail: SevereWeatherDetail | null; loading: boolean; onBack: () => void }) {
  const rc = RISK_CONFIG[point.risk_level as keyof typeof RISK_CONFIG] ?? RISK_CONFIG.LOW
  ...
  return (
    <>
      ...
      <div className={cn("bg-[#0a0d14]/90 backdrop-blur-xl border rounded-lg p-4 shadow-2xl", rc.border)}>
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-white/40 text-[10px] font-mono mt-0.5">SSPI Score: {point.score}/100</p>
          </div>
          <span className={cn("text-2xl font-black", rc.color)}>{point.score}</span>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-black/30 rounded border border-white/5 p-2">
            ...
            <p className="text-sm font-black text-white leading-tight">{point.wind_gusts_10m?.toFixed(0) ?? "—"}<span className="text-[10px] text-white/40 ml-0.5">km/h</span></p>
          </div>
          <div className="bg-black/30 rounded border border-white/5 p-2">
            <p className="text-[9px] text-white/35 uppercase tracking-widest mb-1">Confidence</p>
            <p className="text-sm font-black text-white leading-tight">{point.confidence}%</p>
          </div>
        </div>
      </div>

      {loading && (
        <div className="bg-[#0a0d14]/90 backdrop-blur-xl border border-cyan-500/20 rounded-lg p-4 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />
          <span className="text-[10px] text-cyan-400/80">Loading detailed analysis...</span>
        </div>
      )}

      {/* Forecast windows */}
      {detail && detail.forecast_risk.length > 0 && (
        <div className="bg-[#0a0d14]/90 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-white/[0.06] flex items-center gap-2">
            <Activity className="w-3.5 h-3.5 text-white/40" />
      ...
      {/* Active alerts */}
      {detail && detail.active_alerts.length > 0 && (
      ...
      {/* Impact corridor */}
      {detail && detail.forecast_risk[0]?.impact_corridor && (
        <div className="bg-[#0a0d14]/90 backdrop-blur-xl border border-white/10 rounded-lg p-4 shadow-2xl">
          <div className="flex items-center gap-2 mb-3">
