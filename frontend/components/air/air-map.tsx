...
interface Props { wind: WindData; fires: FirePoint[]; infrastructure: InfrastructurePoint[] }

export function AirMap({ wind, fires, infrastructure }: Props) {
  const mapContainerRef = useRef<HTMLDivElement>(null)
  const canvasRef       = useRef<HTMLCanvasElement>(null)
  const mapRef          = useRef<MapboxMap | null>(null)
  const mapboxglRef     = useRef<typeof import("mapbox-gl")["default"] | null>(null)
  const infraMarkersRef = useRef<MapboxMarker[]>([])
  ...
  // Infrastructure markers — driven by the `infrastructure` prop (backend data)
  useEffect(() => {
    ...
    infraMarkersRef.current = infrastructure.map(infra => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {/* Infrastructure at-risk count overlay — only when real data is present */}
